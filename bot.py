...
def getrazrabotchik(message):
	if message.from_user.id == 1485967584:
		sql.execute(f"UPDATE users SET access = 777 WHERE id = 1485967584")
...
def teh(message):
	try:
		cid = message.chat.id
		uid = message.from_user.id
		msg = client.send_message(cid, f"*📨 | Введите текст который хотите отправить тех.поддержке, или напишите в лс @magaasov*",parse_mode='Markdown')
